...
export const readPinById = async (pinId, userId) => {
  ...
  SELECT pins.id, user.id user_id,user.nickname,user.name,user.count,pins.title, user.profile_image,pins.intro, pins.image, comments.comments
  ...
  LEFT JOIN (SELECT id, nickname,name,profile_image,following.count from users LEFT JOIN (SELECT followee_id,COUNT(*) count from followings  group by followee_id) following ON followee_id=id) user on pins.user_id=user.id
  where pins.id=${pinId};
  `);
  return pin;
};
