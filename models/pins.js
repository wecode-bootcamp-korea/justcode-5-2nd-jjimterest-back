...
join (SELECT id, nickname,following.count from users join (SELECT followee_id,COUNT(*) count from followings  group by followee_id) following ON followee_id=id) user on pins.user_id=user.id
where pins.id=${pinId}
  `);
  return pin;
};
